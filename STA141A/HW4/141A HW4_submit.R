...
Davis_bed = Davis[,c('longitude','la?itude','bedrooms')]
...
p3 = ggmap(m) + geom_jitter(aes(x = longitude, y = latitude, colour = bedrooms),data = Davis_bed,
                            height = 0.001, width = 0.00?,size = 3,alpha = 0.85) +
  ...
  theme(legend.position="top",plot.title = element_text(hjust = 0.5,size = 16),
        legend.text = element_text(size=15),legend.title = element_?ext(size=14))+ 
...
p4 = ggmap(m) + g?om_jitter(aes(x = longitude, y = latitude, colour = bathrooms),data = Davis_bath,
...
p6 = ggmap(m) + geom_jitter(aes(x = longitude, y = l?titude, colour = laundry),data = Davis_laundry,
                            height = 0.001, width = 0.001,size = 3,alpha = 0.85) +
  labs(title = "Distribution of Apartments Laundry in Davis", x='',y='')+ 
  theme(legend.position="top",plot.title = element?text(hjust = 0.5,size = 20),
        legend.text = element_text(size=15),legend.title = element_text(size=15))

Davis_pets = Davis[,c('longitude','latitude','pets')]
Davis_pets$pets = as.factor(Davis_pets$pets)
Davis_pets = na.omit(Davis_pets)
p7 = ggmap(m? + geom_jitter(aes(x = longitude, y = latitude, colour = pets),data = Davis_pets,
                            height = 0.001, width = 0.001,size = 3,alpha = 0.85) +
  labs(title = "Distribution of Apartments Pets Policy in Davis", x='',y='')+ 
  theme(lege?d.position="top",plot.title = element_text(hjust = 0.5,size = 20),
        legend.text = element_text(size=15),legend.title = element_text(size=15))
...
D_suit = Davis[whi?h(Davis$bathrooms >= Davis$bedrooms),]
...
ggmap(m) + geom_jitter(aes(? = longitude, y = latitude),data = D_suit,
...
SBA = apartments[apartments[,'county']%in%
                   c('San Francisco', 'San Mateo', 'Sant? Clara', 'Alameda','Contra Costa'),]
SBA = na.omit(SBA)
...
# family friendly 
S_ff = SBA[SBA[,'?edrooms']%in%c('3','4','5','6'),]
...
g2 = ggmap(p, extent = "panel", maprange=FALSE) +
  ...
  theme(legend.positi?n = "none", axis.title = element_blank(),plot.title = element_text(hjust = 0.5,size = 13),
        axis.text = element_blank(),axis.ticks = element_blank()) +
  labs(title = 'Distribution of Family Friendly Apartments in Southern Bay Area')


# not friendl?
S_ff_no = SBA[SBA[,'bedrooms']%in%c('0','1','2'),]
...
S_ff_no = S_f?_no[S_ff_no[,'laundry']%in%c('none','shared'),]
...
g1 = ggmap(p, extent = "panel", maprange=FALSE) +
  ...
  sta?_density2d(data = S_ff_no, aes(x = longitude, y = latitude, fill = ..level.., alpha = ..level..),
                 size = 0.01, bins = 30, geom = 'polygon') +
  scale_fill_gradient(low = "green", high = "red") +
  scale_alpha(range = c(0.00, 0.25), guide =?FALSE) +
  theme(legend.position = "none", axis.title = element_blank(),plot.title = element_text(hjust = 0.5,size = 13),
        axis.text = element_blank(),axis.ticks = element_blank()) +
  labs(title = 'Distribution of Not Family Friendly Apartments in ?outhern Bay Area')
...
SBA_sqft$quartile = as.integer(cut(S?A_sqft$pre, quantile(SBA_sqft$pre, probs=0:4/4), include.lowest=TRUE))
...
#  labs(title = "Distribution of?Apartments Size in Quartile : Southern Bay Area", x='',y='')+ guides(colour=guide_legend(title="Size Quartile")) +
...
#SBA_price$quartile = as.integer(cut(SBA_?rice$price, quantile(SBA_price$price, probs=0:4/4), include.lowest=TRUE))
#SBA_price$quartile = as.factor(SBA_price$quartile)

#ggmap(p, extent = "panel", maprange=FALSE) +
#  geom_density2d(data = SBA_price, aes(x = longitude, y = latitude)) +
#  stat_den?ity2d(data = SBA_price, aes(x = longitude, y = latitude, fill = ..level.., alpha = ..level..),
#                 size = 0.01, bins = 30, geom = 'polygon') +
#  scale_fill_gradient(low = "green", high = "red") +
#  scale_alpha(range = c(0.00, 0.25), guide =?FALSE) +
#  theme(legend.position = "none", axis.title = element_blank(),plot.title = element_text(hjust = 0.5,size = 15)) +
#  labs(title = 'Distribution of Apartments Price by Quartile in Southern Bay Area') + facet_wrap(~quartile)
# get price quartile
#?uantile(SBA_price$price, c(0,0.25, 0.5, 0.75,1), type = 1) 
...
MyMap <- get_map(l?cation = c(lon = mean(lon), lat = mean(lat)), 
...
ggmap(MyMap) %+% SBA_price +
  ...
  scale_fill_g?adientn(name = "Price",colours = YlOrBr, space = "Lab") +
...
SBA$quartile = as.factor(SBA$quartile)
quan?ile(SBA$pre, c(0,0.25, 0.5, 0.75,1), type = 1,na.rm = T) 
# 0.2320205  2.8853914  3.3913043  4.1176471 19.9500000  

S_suit = SBA[which(SBA$bathrooms >= SBA$bedrooms),]
S_suit = S_suit[S_suit[,'parking']%in%c('covered','street','garage','paid'),]
S_suit = ?_suit[S_suit[,'laundry']%in%c('in-unit','shared','hookup'),]
...
ggmap(p) + geom_jitter(aes(x = longitude, y = latitude, colour = quartile),data = S_suit,
  ...
  theme(legend.position="right",plot.title = element_text(hjust = 0.5,size = 14),
        ...
        axis.te?t = element_blank(),axis.ticks = element_blank())+ guides(colour=guide_legend(title="Unit Price Quartile"))+
...
census = census[-1,]
library?stringr)
census[, 3] = str_remove_all(census[, 3], ' (town|city|CDP|CDP \\(.+ County\\)), California')

old = census[,c('GEO.display.label','HD02_S025','HD01_S020')]
colnames(old)[colnames(old)=="GEO.display.label"] <- "city"
colnames(old)[colnames(old)=="?D01_S020"] <- "Total population percent median age"
colnames(old)[colnames(old)=="HD02_S025"] <- "Total population Percent 65+"

SBA_2 = SBA[,c('latitude','longitude','price','sqft','bedrooms','bathrooms','pets','laundry','parking','city','county','pre','q?artile')]
...
M_2$`Total population percent median ag?` = as.numeric(M_2$`Total population percent median age` )
M_2$quartile = as.integer(cut(M_2$`Total population percent median age`, quantile(M_2$`Total population percent median age`, probs=0:4/4,na.rm =T), include.lowest=TRUE))
M_2$quartile = as.factor(M_?$quartile)
...
M$`Total population percent med?an age` = as.numeric(M$`Total population percent median age` )
...
for(i in 1:nrow(M)){
  for(j in 1:?){
    if(M$`Total population percent median age`[i] >= breaks[j] & M$`Total population percent median age`[i] <= breaks[j+1]){M$group[i] = j}
  }
}

M$group = as.factor(M$group)
h1 = ggplot(M,aes(x=price, color = group)) + geom_density(alpha = 0.3,size = ?.5) +
  labs(title = "Distribution of Apartments Price in Southern Bay Area by Age Group") +
...
h2 = ggplot(M,aes(x=sqft, color = group)) + geom_density(alpha = 0.3,size = 1.5) +
  ...
  labs(x=" ") + scale_color_manual(labels = c("31.0 - 36.5", "36.6 - 51.3"),
                                   values=c("#E69F00", "#56B4E9"))+
  theme(legend.background = element_blank(),legend.justification=c(1,1), legend.position=c(1, 1),
        ax?s.ticks.y = element_blank()) + 
  guides(color=guide_legend(title="Age Group")) + xlim(0,3000)

grid.arrange(h1,h2,nrow = 1)

h3 = ggplot(M, aes(x = as.factor(bedrooms) , fill = group)) + 
  geom_bar(position = "dodge")+labs(title = "Number of Bedrooms by ?ge Group", x = "Number of Bedrooms") +
...
h4?= ggplot(M, aes(x = as.factor(pets) , fill = group)) + 
  ...
  theme(axis.ticks.x = element_blank())+theme(plot.title=element_text(hjust=0.5))+
  scale_fill_manual(la?els = c("31.0 - 35.2", "35.3 - 36.2",'36.3 - 38.3','38.4 - 38.8','38.9  -51.3'),
                    values=c("#E69F00", "#56B4E9",'lightslateblue','tomato1','black'))+
  theme(legend.background = element_blank(),legend.justification=c(1,1), legend.positio?=c(1, 1))+
  geom_text(stat='count', aes(label=..count..), vjust=-0.2,position = position_dodge(width = 1))

h5 = ggplot(M, aes(x = as.factor(parking) , fill = group)) + 
  geom_bar(position = "dodge")+labs(title = "Parking Policy by Age Group", x = "Parki?g Policy") +
  theme(axis.ticks.x = element_blank())+theme(plot.title=element_text(hjust=0.5))+
  scale_fill_manual(labels = c("31.0 - 35.2", "35.3 - 36.2",'36.3 - 38.3','38.4 - 38.8','38.9  -51.3'),
                    values=c("#E69F00", "#56B4E9",'light?lateblue','tomato1','black'))+
  theme(legend.background = element_blank(),legend.justification=c(1,1), legend.position=c(1, 1))+
  geom_text(stat='count', aes(label=..count..), vjust=-0.2, position = position_dodge(width = 1))


h6 = ggplot(M, aes(x = as.?actor(laundry) , fill = group)) + 
  geom_bar(position = "dodge")+labs(title = "Laundry Situation by Age Group", x = "Laundry Situation") +
  theme(axis.ticks.x = element_blank())+theme(plot.title=element_text(hjust=0.5))+
  scale_fill_manual(labels = c("3?.0 - 35.2", "35.3 - 36.2",'36.3 - 38.3','38.4 - 38.8','38.9  -51.3'),
                    values=c("#E69F00", "#56B4E9",'lightslateblue','tomato1','black'))+
  theme(legend.background = element_blank(),legend.justification=c(1,1), legend.position=c(1, 1))+?  geom_text(stat='count', aes(label=..count..), vjust=-0.2, position = position_dodge(width = 1))
