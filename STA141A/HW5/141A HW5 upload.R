...
# I have to get all c?lumns information from the posts firstly.
...
# in the txt, the next to last line is the apartment? bathroom, I split the strings by " : "
bathr = function(txt){
...
# in the txt, the third to last li?e is the apartments bedroom, I split the strings by " : "
bedr = function(txt){
...
# in the txt, the 4t? to last line is the apartments longitude, I split the strings by " : "
longi = function(txt){
...
# i? the txt, the 5th to last line is the apartments latitude, I split the strings by " : "
lati = function(txt){
...
# in the txt, the second line to the 8th to last ?ine is the apartments all information
text = function(txt){
...
all_col = function(txt){
  ...
  bedrooms = bedr(txt)
  bathrooms = bathr(txt)
  temp = c(title, ?ext, latitude, longitude,
...
read_all_posts = function(file_name){
  files = list.files(paste('messy_cl/messy/',?ile_name,sep=""), full.names = TRUE)
  ...
  ?ata_frame$longitude = as.numeric(as.character(data_frame$longitude))
...
sand?ego = read_all_posts('sandiego')
...
sfbay_sfc = read_all_posts('sfb?y_sfc')
...
final_data$title_price = rep(0,nrow(fin?l_data))
...
for (i in (1:nrow(final_data))){
  ...
  temp2 = strsplit(temp1,split='$', fixed=TRUE)[[1]][2] # remo?e "$"
...
temp1 = na.omit(temp1)
not_equal = temp1[!temp1$pric?==temp1$title_price,]
...
for (i in (1:n?ow(final_data))){
  ...
  temp6 = str_extract(temp5, "\\$[0-9]+") # remove $ and -?  temp6 = str_extract(temp6,"[0-9]+")
...
final_data$d?posit_2[final_data$deposit_2 > 0 & final_data$deposit_2 < 95] = NA

# get no deposit = 0, one month deposit = rent price
final_data$deposit = rep(0,nrow(final_data))
for (i in (1:nrow(final_data))){
  temp1 = final_data$text[i]
  # extract lines have 'depo?ite' ,  ignore upper/lower character
  temp2 = str_extract_all(temp1, regex('[\n.!-][^.\n!-]*deposit[^.\n!-]*[\n.!-]?',
                                       ignore_case = TRUE), simplify = T)
  temp3 = str_match(temp2,'(one month rent|1 month rent|One Mo?th Rent|1 Month Rent)')[,1][1]
...
final_data = readRDS('final_data.rds')?# correct errors and limit outliers
...
data_1$deposit_quartile = rep(0, nrow(dat?_1))
...
ggplot(data_1, aes(deposit,p?ice)) + geom_point() + geom_smooth(method='loess') +
  labs(title="Relationship of Apartments Deposit and Rental Price by Deposit Quartile",x="Apartments Deposit",y="Apartments Rental Price")+ 
  theme(plot.title=element_text(hjust=0.5,size = 18)) + facet_?rap(~deposit_quartile, scales = "free")
...
get_p?ts = function(comb){
  # extract both pet (cats and dogs)
  temp1_1 = str_detect(comb, regex('welcome (cats? (and|&) dogs?|dogs? (and|&) cats?)', ignore_case = T))
  temp1_2 = str_detect(comb, regex('(cats? (and|&) dogs?|dogs? (and|&) cats?)[ ]?(welcome|al?owed|accepted|-friendly|friendly|ok|okay|are ok|only|accepts)', ignore_case = T))
  ...
  temp1_6 = str_de?ect(comb, regex('allow (cats? (and|&) dogs?|dogs? (and|&) cats?)', ignore_case = T))
  ...
  # ?xtract dogs only and cats only
  ...
  ?lse if(temp2_2 & temp3_1){temp5 = 'dog'}
...
data_3$pet = sapply(data_?$comb, get_pets)
...
ggplot(data = data_3, aes(x=pet_deposit))+
  ...
  labs(title = "Distribution of Pet Deposits") + theme(p?ot.title = element_text(hjust = 0.5,size = 15))+
...
#Small caged animals that are a?lowed are common domesticated household birds, hamsters, gerbils, rabbits, guinea pigs, 
...
# find heating
data_2 = final_data[,c('t?xt','Air_conditioning')]
...
length(which(?ata_2$Heat_Fireplace == 'both')) # 2844
...
fire_heat = data_2[d?ta_2[,'Heat_Fireplace']%in%c('both','Fireplace','Heat'),] #16766
...
length(which(Air_c?n$Heat_Fireplace == 'Fireplace')) # 1510
...
get_email = function(txt){
  temp1 = str_extract(t?t,"[A-z0-9-]+\\@[A-z0-9-]+\\.[A-z0-9-]{0,5}")
...
length(which(data_2$?hone2 == 'phone')) # 34
...
for (i in (1:nrow(data_2))){
  temp1 = data_2$text[i]
  temp2 = st?_detect(temp1, regex('show contact info', ignore_case = TRUE))
  if (temp2 ) { data_2$contact[i] = 'hide'}
  else if(str_detect(temp1, regex(' email| text| phone| call| contact', ignore_case = TRUE))) {data_2$contact[i] = 'not hide'}
